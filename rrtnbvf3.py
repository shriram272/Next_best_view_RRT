...
# # Visualization function to display the explored environment
# def visualize_explored_environment(grid, coverage, best_path=None):
#     fig, ax = plt.subplots()
#     ax.imshow(grid, cmap='gray', alpha=0.5)
#     ax.imshow(coverage, cmap='hot', alpha=0.7)
#     if best_path:
#         path_x, path_y = zip(*best_path)
#         ax.plot(path_x, path_y, 'r', linewidth=2)
#     ax.set_title('Explored Environment')
#     ax.set_xlabel('X-axis')
#     ax.set_ylabel('Y-axis')
#     plt.show()

# # Set up the grid and the start position
# grid = np.load('obstacle_grid8.npy')  # Load your grid here
...
#         info_gain = rrt_nbv.information_gain(newX, newY, theta)
...
#         plt.plot([rrt_nbv.nearest_node.locationX, newX], [rrt_nbv.nearest_node.locationY, newY], 'go', linestyle="--")
#         plt.pause(0.01)

#     # Check if the goal (sufficient coverage) is reached
#     if rrt_nbv.goalReached():
#         print("Goal reached! Extracting best path...")
#         best_path = rrt_nbv.extract_best_path()
#         for j in range(len(best_path) - 1):
#             plt.plot([best_path[j][0], best_path[j + 1][0]], [best_path[j][1], best_path[j + 1][1]], 'r', linewidth=2)
#         plt.pause(0.01)
#         break

# # Visualize the explored environment with the best path highlighted
# visualize_explored_environment(grid, rrt_nbv.coverage, rrt_nbv.extract_best_path())
...
for i in range(rrt_nbv.NT_OL):
    ...
    if len(rrt_nbv.tree) >= rrt_nbv.Nmax:
        if rrt_nbv.best_gain > 0 or not rrt_nbv.continue_tree_construction():
            print("Stopping tree construction.")
            break

# Extract the best path and plot it
best_path = rrt_nbv.extract_best_path()
best_path = np.array(best_path)
plt.plot(best_path[:, 0], best_path[:, 1], 'g-', linewidth=2)
